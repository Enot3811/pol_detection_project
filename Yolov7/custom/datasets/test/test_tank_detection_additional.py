...
# TODO этот датасет не работает с мозаикой
import sys
...
from Yolov7.custom.datasets import TankDetectionDatasetAdditional
...
def main(config_pth: str, dset_type: str):
    ...
    # Get dataset
    obj_dset = TankDetectionDatasetAdditional(
        dset_pth, name2index=config['cls_to_id'],
        polarization=config['polarization'], active_ch=config['active_ch'])
    ...
    for obj_sample, mos_sample, yolo_sample in zip(obj_dset,
        ...
        cv2.imshow('orig', cv2.cvtColor(orig_img_bboxes[:, :, 0],
                   cv2.COLOR_RGB2BGR))
        cv2.imshow('mos_mix', cv2.cvtColor(mos_img_bboxes[:, :, 0],
                   cv2.COLOR_RGB2BGR))
        cv2.imshow('yolo', cv2.cvtColor(yolo_img_bboxes[:, :, 0],
                   cv2.COLOR_RGB2BGR))
        key = cv2.waitKey(0)
        if key == 27:
            break
...
def parse_args() -> argparse.Namespace:
    ...
    args = parser.parse_args([
        'Yolov7/custom/configs/tank_pol_2.json',
        '--dset_type=val'
    ])
